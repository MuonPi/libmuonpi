...
{
    using state_t = muonpi::gpio::state_t;
    state_t state {};

    BOOST_TEST( (state == state_t::Undefined) );

    state = !state;

    BOOST_TEST( (state == state_t::Undefined) );

    state = state_t{ state_t::Low };

    BOOST_TEST( (state == state_t::Low) );

    state = !state;

    BOOST_TEST( (state == state_t::High) );

    state = 3;

    BOOST_TEST( (state == state_t { muonpi::gpio::state_t::High }) );

    BOOST_TEST( (state == state_t::High) );

    int int_state { state };

    BOOST_TEST( (int_state == state_t::High) );
    BOOST_TEST( (int_state == 1) );

    int_state = static_cast<int>(!state);

    BOOST_TEST( (int_state == state_t::Low) );
