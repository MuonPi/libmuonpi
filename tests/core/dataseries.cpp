...
{
    using data_s = muonpi::data_series<double, false>;
...
BOOST_AUTO_TEST_SUITE(core_data_series_test_sample, * utf::tolerance(0.00001))

BOOST_AUTO_TEST_CASE(empty_test)
{
    using data_s = muonpi::data_series<double, true>;
...
BOOST_AUTO_TEST_CASE(fill_test)
{
    ...
    BOOST_TEST(series.variance() == 0.16666666);
    BOOST_TEST(series.current() == 0.5);
    BOOST_TEST(series.min() == 0.5);
    BOOST_TEST(series.max() == 1.5);
    BOOST_TEST(series.sum() == 10.0);
