...
BOOST_AUTO_TEST_CASE(test_startup) {
    ...
    BOOST_TEST(fixture_thread_runner::runner.wait_for(muonpi::thread_runner::State::Running,
                                                      std::chrono::seconds {1}));
}
BOOST_AUTO_TEST_CASE(test_shutdown) {
    fixture_thread_runner::runner.stop(0);
    std::this_thread::sleep_for(std::chrono::milliseconds {100});
    BOOST_TEST(fixture_thread_runner::runner.wait_for(muonpi::thread_runner::State::Stopped,
                                                      std::chrono::seconds {1}));
...
BOOST_AUTO_TEST_CASE(test_initial_state) {
    ...
    BOOST_TEST(
        (fixture_thread_runner_failing::runner.state() == muonpi::thread_runner::State::Initial));
}
BOOST_AUTO_TEST_CASE(test_run) {
    fixture_thread_runner_failing::runner.start();
    BOOST_TEST(fixture_thread_runner_failing::runner.wait_for(muonpi::thread_runner::State::Running,
                                                              std::chrono::seconds {1}));
    BOOST_TEST(fixture_thread_runner_failing::runner.wait_for(muonpi::thread_runner::State::Error,
