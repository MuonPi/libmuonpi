...
protected:
    ...
    std::array<bool, 4>       m_agc {false,
                               false,
                               false,
                               false}; ///< software agc which switches over to a better pga setting
                                             ///< if voltage too low/high
    bool m_diff_mode {
        false}; ///< measure differential input signals (true) or single ended (false=default)
    CONV_MODE             m_conv_mode {CONV_MODE::UNKNOWN};
    std::array<Sample, 4> m_last_sample {InvalidSample,
                                         InvalidSample,
                                         InvalidSample,
                                         InvalidSample};
...
auto ADS1115::setThreshold(std::int16_t threshold) -> bool {
    static_assert((R == ADS1115::REG::LO_THRESH) || (R == ADS1115::REG::HI_THRESH),
