...
/**
 * @brief The Ecef struct, coordinate data in the Ecef reference system. Earth-relative carthesian
 * coordinates
 */
template <typename T>
...
/**
 * @brief The Enu struct, coordinate data in the Enu reference system. Carthesian coordinates
 * relative to other Ecef coordinates.
 */
template <typename T>
...
public:
    [[nodiscard]] static auto from_geodetic(const geodetic<T>& coords, std::size_t precision)
        -> std::string;

private:
    static constexpr const char* base32 {
        "0123456789bcdefghjkmnpqrstuvwxyz"}; // (geohash-specific) base32 map
};

template <typename T, template <typename MT = T> typename Model>
/**
 * @brief The Model class, prototype for conversion between all three coordinate systems.
 * Implementations can be created depending on the specific earth Model used.
 */
...
public:
     ...
     * @return
     */
    [[nodiscard]] static auto to_geodetic(const enu<T>& coords, const ecef<T>& reference)
        -> geodetic<T>;
     ...
     * @return
     */
    [[nodiscard]] static auto straight_distance(const geodetic<T>& first, const geodetic<T>& second)
        -> T;
};
...
auto transformation<T, Model>::to_ecef(const geodetic<T>& coords) -> ecef<T> {
    const T N {Model<T>::a
               / std::sqrt(1.0 - Model<T>::e_squared * std::pow(std::sin(coords.lat), 2.0))};
    return {(N + coords.h) * std::cos(coords.lat) * std::cos(coords.lon),
            (N + coords.h) * std::cos(coords.lat) * std::sin(coords.lon),
            (N * std::pow(Model<T>::b, 2.0) / std::pow(Model<T>::a, 2.0) + coords.h)
                * std::sin(coords.lat)};
}
...
auto transformation<T, Model>::to_ecef(const enu<T>& coords, const ecef<T>& reference) -> ecef<T> {
    ...
    return {
        (-std::sin(ref_g.lon) * coords.x - std::sin(ref_g.lat) * std::cos(ref_g.lon) * coords.y
         + std::cos(ref_g.lat) * std::cos(ref_g.lon) * coords.z)
            + reference.x,
        (std::cos(ref_g.lon) * coords.x - std::sin(ref_g.lat) * std::sin(ref_g.lon) * coords.y
         + std::cos(ref_g.lat) * std::sin(ref_g.lon) * coords.z)
            + reference.y,
...
template <typename T, template <typename MT = T> typename Model>
auto transformation<T, Model>::to_enu(const geodetic<T>& coords, const ecef<T>& reference)
    -> enu<T> {
...
auto transformation<T, Model>::to_enu(const ecef<T>& coords, const ecef<T>& reference) -> enu<T> {
    ...
    return {-std::sin(ref_g.lon) * d_x + std::cos(ref_g.lon) * d_y,
            -std::sin(ref_g.lat) * std::cos(ref_g.lon) * d_x
                - std::sin(ref_g.lat) * std::sin(ref_g.lon) * d_y + std::cos(ref_g.lat) * d_z,
            std::cos(ref_g.lat) * std::cos(ref_g.lon) * d_x
                + std::cos(ref_g.lat) * std::sin(ref_g.lon) * d_y + std::sin(ref_g.lat) * d_z};
}

template <typename T, template <typename MT = T> typename Model>
auto transformation<T, Model>::to_geodetic(const enu<T>& coords, const ecef<T>& reference)
    -> geodetic<T> {
...
auto transformation<T, Model>::to_geodetic(const ecef<T>& coords) -> geodetic<T> {
    ...
    const double e_squared {(std::pow(Model<T>::a, 2.0) - std::pow(Model<T>::b, 2.0))
                            / std::pow(Model<T>::b, 2.0)};
    const double F {54.0 * std::pow(Model<T>::b, 2.0) * std::pow(coords.z, 2.0)};
    const double G {std::pow(r, 2.0) + (1.0 - Model<T>::e_squared) * std::pow(coords.z, 2.0)
                    - Model<T>::e_squared
                          * (std::pow(Model<T>::a, 2.0) - std::pow(Model<T>::b, 2.0))};
    ...
    const double r_0 {
        -P * Model<T>::e_squared * r / (1.0 + Q)
        + std::sqrt(0.5 * std::pow(Model<T>::a, 2.0) * (1.0 + 1.0 / Q)
                    - P * (1.0 - Model<T>::e_squared) * std::pow(coords.z, 2.0) / (Q * (1.0 + Q))
                    - 0.5 * P * std::pow(r, 2.0))};
    const double U {
        std::sqrt(std::pow(r - Model<T>::e_squared * r_0, 2.0) + std::pow(coords.z, 2.0))};
    const double V {std::sqrt(std::pow(r - Model<T>::e_squared * r_0, 2.0)
                              + (1.0 - Model<T>::e_squared) * std::pow(coords.z, 2.0))};
    const double z_0 {std::pow(Model<T>::b, 2.0) * coords.z / (Model<T>::a * V)};
    return {std::atan((coords.z + e_squared * z_0) / r),
            std::atan2(coords.y, coords.x),
            U * (1.0 - std::pow(Model<T>::b, 2.0) / (Model<T>::a * V))};
}

template <typename T, template <typename MT = T> typename Model>
auto transformation<T, Model>::straight_distance(const geodetic<T>& first,
                                                 const geodetic<T>& second) -> T {
    const auto second_enu {to_enu(second, to_ecef(first))};
    return std::sqrt(std::pow(second_enu.x, 2.0) + std::pow(second_enu.y, 2.0)
                     + std::pow(second_enu.z, 2.0));
}

template <typename T>
[[nodiscard]] auto hash<T>::from_geodetic(const geodetic<T>& coords, std::size_t precision)
    -> std::string {
