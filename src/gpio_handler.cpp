...
auto gpio_handler::set_pin_interrupt(const gpio::pin_setting_t& settings,
    const gpio::callback_t& callback) -> bool
{
    ...
    if (it == m_callback.end()) {
        ...
        log::debug() << "Registered event callback for pin " << settings.pin << " '"
                     << m_chip.lines.at(settings.pin).name << "'";
        return true;
    }
...
auto gpio_handler::set_pin_interrupt(const gpio::pins_t& pins, const gpio::callback_t& callback)
    -> bool
{
...
auto gpio_handler::set_pin_output(gpio::pin_t pin, gpio::state_t initial_state, gpio::bias_t bias)
    -> std::function<bool(gpio::state_t)>
{
    ...
    int ret = gpiod_line_request_output_flags(line,
        m_consumer.c_str(),
        get_flags(bias),
        static_cast<int>(initial_state));

    if (ret < 0) {
        log::error() << "Request gpio line " << pin
                     << " as output failed: " << std::strerror(errno);
    ...
    return [pin, this](gpio::state_t state) {
        ...
        if (r < 0) {
            log::error() << "Setting state of gpio line " << pin
...
auto gpio_handler::get_pin_input(gpio::pin_t pin, gpio::bias_t bias)
    ...
    while (!m_quit) {
        ...
        if (ret > 0) {
            for (std::size_t i { 0 }; i < event_bulk.num_lines; i++) {
                ...
                m_events.emplace(
                    private_event { gpiod_line_offset(event_bulk.lines[i]), line_event });
            }
...
auto gpio_handler::pre_run() -> int
{
    ...
    if (m_interrupt_condition.wait_for(lock, std::chrono::seconds { 5 }) == std::cv_status::timeout
        && m_interrupt_lines.empty()) {
        return 1;
    }
    ...
    m_callback_thread = std::thread { [&]() {
        while (!m_quit) {
            ...
            while (!m_events.empty()) {
                ...
                evt.time = gpio::time_t { std::chrono::seconds { event.evt.ts.tv_sec }
                    + std::chrono::nanoseconds { event.evt.ts.tv_nsec } };

                m_event_rate.increase_counter();

                // Since no interrupts are registerred without callback, we always know that there
                // is an associated callback
            ...
            if (m_event_rate.step()) {
                const float timeout_us {
                    std::clamp(s_m * m_event_rate.mean() + s_b, 0.0F, s_max_timeout)
                };
