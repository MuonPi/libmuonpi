...
public:
    explicit session(tcp::socket&& socket,
        ssl::context& ctx,
        std::function<response_type(request_type)> handler);
...
template <>
session<ssl_stream_t>::session(tcp::socket&& socket,
    ssl::context& ctx,
    std::function<response_type(request_type)> handler)
...
template <>
session<tcp_stream_t>::session(tcp::socket&& socket,
    std::function<response_type(request_type)> handler)
...
void session<Stream>::on_read(beast::error_code errorcode, std::size_t bytes_transferred)
{
    ...
    auto sp { std::make_shared<beast::http::message<false, beast::http::string_body>>(
        m_handler(std::move(m_req))) };

    bool close { sp->need_eof() };
    m_res = sp;
    beast::http::async_write(m_stream, *sp, [&](beast::error_code ec, std::size_t bytes) {
