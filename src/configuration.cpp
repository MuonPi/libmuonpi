...
auto config::initialisation::add_option(const std::string& name, const std::string& description)
    -> initialisation&
{
...
{
    boost::program_options::store(boost::program_options::parse_command_line(argc, argv, m_desc),
        m_options);
    s_instances--;
    if (s_instances == 0) {
...
void config::initialisation::commit(const std::string& filename)
{
    ...
    if (ifs) {
        boost::program_options::store(boost::program_options::parse_config_file(ifs, m_desc),
            m_options);
    } else {
...
config::initialisation::initialisation(const std::string& name,
...
auto operator<<(std::ostream& ostream, const config::initialisation& initialisation)
    -> std::ostream&
{
