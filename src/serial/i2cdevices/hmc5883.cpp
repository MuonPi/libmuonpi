...
{
    std::uint8_t bitmask = static_cast<std::uint8_t>(m_config) | static_cast<std::uint8_t>(m_averages);
    if ( write( static_cast<std::uint8_t>(Reg::ConfigA), &bitmask, 1u ) != 1 ) return false;
    return true;
}

auto HMC5883::init() -> bool
{
    if ( !write_config() || !set_gain( m_gain ) ) return false;
    return true;
}

auto HMC5883::set_gain(std::uint8_t gain) -> bool
{
    if ( gain > 0x07 ) return false;
    std::uint8_t bitmask = gain << 5u;
    if ( write( static_cast<std::uint8_t>(Reg::ConfigB), &bitmask, 1u ) != 1 ) return false;
    m_gain = gain;
    return true;
}
...
auto HMC5883::getXYZRawValues() -> std::optional<std::vector<int>>
{
    ...
    if ( write( static_cast<std::uint8_t>(Reg::Mode), &start_cmd, 1u ) != 1 ) { return std::nullopt; }
    
    std::this_thread::sleep_for(std::chrono::milliseconds(6));

    std::array<std::uint16_t, 3> data_buffer { 0, 0, 0 };
    if ( read( static_cast<std::uint8_t>(Reg::Data), data_buffer.data(), 3u ) != 3 ) { return std::nullopt; }
    
    // generate the x,y,z entries: cast to signed integer and reorder, since the readout sequence is x,z,y
    std::vector<int> values { 
        static_cast<std::int16_t>(data_buffer[Axis::X]),
        static_cast<std::int16_t>(data_buffer[Axis::Z]),
        static_cast<std::int16_t>(data_buffer[Axis::Y])
    };

    if ( values[Axis::X] >= -2048 && values[Axis::X] < 2048 
        && values[Axis::Y] >= -2048 && values[Axis::Y] < 2048 
        && values[Axis::Z] >= -2048 && values[Axis::Z] < 2048)
    {
...
auto HMC5883::get_magnetic_field_vector() -> std::optional<magnetic_field_t>
{
    ...
    if ( !raw_vector ) return std::nullopt;

    magnetic_field_t field_vector;
    std::transform(raw_vector->begin(), raw_vector->end(), std::back_inserter(field_vector),
...
{
    double square_sum = std::accumulate( b_field.cbegin(), b_field.cend(), 0, [](double sum, double axis_value) {
...
auto HMC5883::identify() -> bool
{
    ...
    if ( read(static_cast<std::uint8_t>(Reg::ID), id_buffer.data(), 3) != 3 ) return false;
