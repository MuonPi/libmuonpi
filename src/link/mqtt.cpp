...
void mqtt::callback_message(const mosquitto_message* message)
{
    ...
    for (auto& [topic, sub] : m_subscribers) {
        bool result {};
        mosquitto_topic_matches_sub2(topic.c_str(),
            topic.length(),
            message_topic.c_str(),
            message_topic.length(),
            &result);
        if (!result) {
            continue;
        }
...
auto mqtt::publish(const std::string& topic, const std::string& content) -> bool
{
    ...
    auto result { mosquitto_publish(m_mqtt,
        nullptr,
        topic.c_str(),
...
auto mqtt::p_subscribe(const std::string& topic) -> bool
{
    ...
    if (result != MOSQ_ERR_SUCCESS) {
        switch (result) {
        case MOSQ_ERR_INVAL:
            log::error("mqtt") << "Could not subscribe to topic '" << topic
                               << "': invalid parameters";
            break;
        case MOSQ_ERR_NOMEM:
...
auto mqtt::connect() -> bool
{
    ...
    if (m_connect_condition.wait_for(lock, std::chrono::seconds { m_tries })
    ...
    if (mosquitto_username_pw_set(m_mqtt,
            m_config.login.username.c_str(),
            m_config.login.password.c_str())
        != MOSQ_ERR_SUCCESS) {
        log::warning("mqtt") << "Could not set username and password.";
        return false;
    }
    auto result {
        mosquitto_connect(m_mqtt, m_config.host.c_str(), m_config.port, m_config.keepalive)
    };
    if (result == MOSQ_ERR_SUCCESS) {
        m_connect_promise = std::promise<bool> {};
        m_connect_future = m_connect_promise.get_future();
        if ((m_connect_future.wait_for(m_config.timeout) == std::future_status::ready)
            && m_connect_future.get()) {
            return true;
        }
    } else {
...
auto mqtt::publisher::publish(const std::string& subtopic, const std::vector<std::string>& content)
    -> bool
{
    std::ostringstream stream {};
    for (const auto& string : content) {
