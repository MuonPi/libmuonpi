...
auto main() -> int
{
    ...
    if (ok) {
        ...
        log::info()<<"identified "<< tempsensor.name() <<" at 0x" << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>( tempsens_addr ) <<" : temp=" << std::dec << tempsensor.get_temperature();
    } else {
    ...
    if (ok) {
        ...
        log::info()<<"identified "<<adc.name()<<" at 0x" 
    ...
    if ( bus.identify_device<serial::devices::eeproms::MC24AA02UID>( eeprom_addr ) ) {
        // found the specific device at the expected position, so close the previously created generic
        // i2c_device and reopen as eeprom device
        ...
        log::info()<<"identified "<<eep.name()<<" at 0x"
        << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>( eeprom_addr );
        ...
        while ( page < eep.size()/eep.page_size() ) {
            ...
            for ( std::size_t page_addr { 0 };page_addr<eep.page_size();page_addr++ ) {
                ostr << ' ' << std::hex << std::setw(2) << std::setfill('0') << static_cast<std::uint16_t>(page_buffer.at(page_addr));
            }
            log::info() << ostr.str();
            page++;
        }
